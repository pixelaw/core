...
pub mod paint_actions {
    ...
    impl ActionsImpl of IPaintActions<ContractState> {
        ...
        fn interact(ref self: ContractState, default_params: DefaultParameters) {
            let world = self.world(@"pixelaw");
            let position = default_params.position;

            // Load the Pixel
            let mut pixel: Pixel = world.get_model((position.x, position.y));
        ...
        fn fade(ref self: ContractState, default_params: DefaultParameters) {
            ...
            let pixel: Pixel = world.get_model((position.x, position.y));

            let (r, g, b, a) = decode_rgba(pixel.color);

            // If the color is 0,0,0, fading is done.
            if r == 0 && g == 0 && b == 0 {
                world.erase_model(@(position.x, position.y));
                return;
            }
